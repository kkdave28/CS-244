...
main(int argc, char *argv[])
{
   ...
   ****************************************************************************/
   sprintf(outfilename, "%s_s_%3.2f_l_%3.2f_h_%3.2f.pgm", argv[5],
      sigma, tlow, thigh); //take one more input for output file. 
...
/* Modifications done by me (Kush Dave)
*/
        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   ...
   for(i=0; i<N_T; i++)
   {
      ...
      argsy[i].rows = rows;
      argsy[i].cols = cols;
      argsy[i].row_s = row_per_t*i;
      argsy[i].row_e = row_per_t*(i+1);
      argsy[i].center = center;
      argsy[i].kernel = kernel;
      argsy[i].tempim = tempim;
      argsy[i].smoothness = smoothedim;
      iret[i] = pthread_create(&thread[i], NULL, &blur_y, &argsy[i]);
   }
